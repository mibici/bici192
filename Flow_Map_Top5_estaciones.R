...
SELECT Origen_Id, B.Latitude, B.Longitude,  Destino_Id, D.Latitude, D.Longitude, COUNT(Viaje_Id) AS Viajes FROM
	...
	WHERE 
    Origen_Id = '50' OR Destino_Id = '50' OR
    Origen_Id = '51' OR Destino_Id = '51' OR
    Origen_Id = '75' OR Destino_Id = '75' OR
    Origen_Id = '54' OR Destino_Id = '54' OR
    Origen_Id = '52' OR Destino_Id = '52'
...
ggplot() + 
  ...
  geom_text(aes(-103.34616, 20.67663, label = 'Guajadalara'), size = 1.5, color = "white")+
  geom_text(aes(-103.3847885, 20.7235603, label = 'Zapopan'), size = 1.5, color = "white")+
  geom_curve(data = consulta6, aes(x = Longitude, y = Latitude, xend = Longitude..6+.01, yend = Latitude..5),
             size=0.00125, color = "white", alpha = consulta6$Viajes/2500,curvature = -0.25, arrow = arrow(length = unit(0.005, "npc"))) +
  ...
  coord_cartesian(ylim=c(20.638,20.74),xlim=c(-103.41,-103.295))
